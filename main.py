...
class OmsCli(object):
    def upsert(self):
        ...
        ret = subprocess.run(
            [
                ...
                "-format=json",
                "-mount=secret",
                "oms/{}".format(result[0][1]),
                "password={}".format(result[1][1]),
                "description={}".format(result[2][1]),
            ],
            capture_output=True,
        )
        ...
        console.print(
            "[green]Finish upsert server: [/green][sky_blue3]{}[/sky_blue3]".format(
                result[0][1]
            )
        )
    ...
    def list(self):
        ret = subprocess.run(
            ["vault", "kv", "list", "-format=json", "secret/oms"], capture_output=True
        )
        data = json.loads(ret.stdout.decode("utf-8"))

        console.print(
            "[bold medium_purple]All servers are listed:[/bold medium_purple]"
        )
        for k in data:
            ret = subprocess.run(
                [
            ...
            console.print(
                "[sky_blue3]{}[/sky_blue3]\t{}".format(
                    k, json.loads(ret.stdout.decode("utf-8"))
                )
            )
    ...
    def ssh(self):
        ...
        data = json.loads(ret.stdout.decode("utf-8"))
        cli = Bullet(prompt="Choose from the servers below:", choices=data)
        result = cli.launch()
